...
public class StreamingMusica {
    ...
    // ─── Lista polimórfica: armazena Free e Premium no mesmo ArrayList ───
    ...
    public static void main(String[] args) {
        ...
        do {
            if (usuarioLogado == null) {
                ...
                // Polimorfismo: instanceof determina qual menu exibir
                ...
                // Casting: acessa comportamentos específicos de cada subtipo
                if (usuarioLogado instanceof UsuarioPremium) {
    ...
    static void criarUsuario() {
        ...
        while (true) {
            ...
            // Verifica e-mail duplicado (instanceof + casting para acessar getEmail())
            ...
            if (duplicado) { System.out.println("E-mail ja cadastrado."); continue; }
            break;
        }
    ...
    static void fazerLogin() {
        ...
        try {
            ...
            // instanceof para exibir mensagem específica
            if (usuarioLogado instanceof UsuarioPremium) {
                UsuarioPremium p = (UsuarioPremium) usuarioLogado;
                System.out.println("Login realizado: " + p.getNome()
                        + " (Premium - " + p.getTipoPlano() + ")");
            } else if (usuarioLogado instanceof UsuarioFree) {
                UsuarioFree f = (UsuarioFree) usuarioLogado;
                System.out.println("Login realizado: " + f.getNome() + " (Free)");
            }
        } catch (Exception e) {
    ...
    static void listarUsuarios() {
        ...
        for (int i = 0; i < usuarios.size(); i++) {
            ...
            // instanceof + downcasting para acessar atributos específicos
    ...
    // ─────────────────────────────────────────────
    // ESTATÍSTICAS DO SISTEMA (instanceof + casting)
    // ─────────────────────────────────────────────
    ...
    static void exibirEstatisticas() {
        ...
        int anunciosTotais = 0;
        int totalDownloads = 0;

        // Percorre lista polimórfica usando instanceof para diferenciar tipos
        for (Usuario u : usuarios) {
            if (u instanceof UsuarioPremium) {
                totalPremium++;
                ...
                totalFree++;
                UsuarioFree f = (UsuarioFree) u; // downcasting
        ...
        System.out.println();
        System.out.println("Anuncios exibidos (usuarios Free): " + anunciosTotais);
        System.out.println("Downloads realizados (Premium):    " + totalDownloads);
    ...
    static void menuPlaylistsAutomaticas(Usuario u) {
        ...
        u.playlists.add(pa);
        System.out.println("Playlist automatica adicionada ao seu perfil!");

        System.out.print("Reproduzir agora? (1=Sim / outro=Nao): ");
        try {
            if (Integer.parseInt(scanner.nextLine()) == 1) {
                pa.reproduzir(); // polimorfismo: PlaylistAutomatica.reproduzir()
            }
        } catch (Exception e) {}
    }
    ...
    static void fazerUpgradePremium(UsuarioFree free) {
        ...
        // Substitui o usuário Free pelo Premium na lista
    ...
    // Método polimórfico: funciona para Free e Premium via referência Usuario
    static void reproduzirMusica(Usuario u) {
        ...
        try {
            ...
            if (idx >= 0 && idx < bancoMusicas.size()) {
                u.reproduzirMusica(bancoMusicas.get(idx)); // polimorfismo!
            } else {
    ...
    static void baixarMusicaPremium(UsuarioPremium u) {
        ...
        try {
            int idx = Integer.parseInt(scanner.nextLine()) - 1;
            if (idx >= 0 && idx < bancoMusicas.size()) {
                u.baixarMusica(bancoMusicas.get(idx));
            } else {
                System.out.println("Musica nao encontrada.");
            }
        } catch (Exception e) {
    ...
    static void criarPlaylist(Usuario u) {
        ...
        try {
            u.criarPlaylist(nome); // polimorfismo: Free limita a 3, Premium ilimitado
        } catch (IllegalArgumentException e) {
    ...
    static void gerenciarPlaylists(Usuario u) {
        ...
        if (op == 2) {
            // instanceof: PlaylistAutomatica não aceita músicas manuais
            ...
            // instanceof para exibir detalhes específicos do subtipo
            if (p instanceof PlaylistAutomatica) {
                PlaylistAutomatica pa = (PlaylistAutomatica) p;
                System.out.println("Criterio: " + pa.getCriterio());
            } else if (p instanceof PlaylistPersonalizada) {
                PlaylistPersonalizada pp = (PlaylistPersonalizada) p;
                System.out.println("Criada por: " + pp.getCriadorNome());
            }
        } else if (op == 5) {
            p.reproduzir(); // polimorfismo: cada subtipo exibe de forma diferente
        }
    }
